...
- Les correspondances entre en-têtes originaux et en-têtes standard sont stockées dans `chronogram_pipeline/config/mapping_headers.csv` (format "En-tete original,En-tete standard").
...
- Les valeurs à normaliser dans certaines colonnes sont gérées par `chronogram_pipeline/config/mapping_values.csv`. Ce fichier comporte trois colonnes : `Colonne`, `Valeur brute` et `Valeur standard`.
- Ajoutez une ligne pour chaque nouvelle valeur à uniformiser. Par exemple :
  ...
  ```
- Une version YAML (`chronogram_pipeline/config/value_mappings.yaml`) peut également être utilisée par `standardizer.py` pour stocker ces correspondances.
...
Le fichier `chronogram_pipeline/config/schema_definition.yaml` définit, lorsque présent, la liste des champs standards attendus ainsi que les valeurs autorisées pour certaines colonnes. Mettez ce fichier à jour si de nouveaux champs doivent être pris en compte.
